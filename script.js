  ...
  reader.onload = function (e) {
    ...
    lines.forEach((line, index) => {
      if (index > 0 && line) {
        ...
        const [timeValue, pressureValue, flowrateValue] = line
          .split(",")
...
function handleSecondCSV(file) {
  ...
  reader.onload = function (e) {
    ...
    const values = lines[1]
      ? lines[1].split(",").map((value) => value.trim())
      : [];
    ...
    if (displayStyle !== "none") {
      if (window.matchMedia("(max-width: 576px)").matches) {
        ...
        chartcontainer.style.height =
          (chartcontainer.offsetWidth * 5) / 6 + "px";
        ...
        chartcontainer.style.width = "100%";
        chartcontainer.style.height =
          (chartcontainer.offsetWidth * 5) / 8 + "px";
        chartcontainer.style.display = "flex";
        chartcontainer.style.justifyContent = "center";
        chartcontainer.style.alignItems = "center";
      } else {
        ...
        chartcontainer.style.width = "100%";
        chartcontainer.style.height =
          (chartcontainer.offsetWidth * 5) / 12 + "px";
        chartcontainer.style.display = "flex";
        chartcontainer.style.justifyContent = "center";
        chartcontainer.style.alignItems = "center";
      }
...
function smallScreen() {
  if (window.matchMedia("(max-width: 576px)").matches) {
    alert(
      "Switch to landscape mode and **refresh** the page to have a better view of the chart.\n\nIt still works if you do not!"
    );
  }
}
...
function hornerPlot(time, Pressure_, arrayOfObjectsSheet2) {
  ...
  if (typeof time_of_production === "undefined") {
    time_of_production =
      (24 * Calculation_data_array[0].cumulative_production) /
      Calculation_data_array[0].Flow_rate;
  ...
  function Semi_LogGraph(time, pressure_data) {
    ...
    const svg = d3
      .select("#chart-container") // I selected "#chart-container" as it is because d3 uses CSS elements to select html elements.
      .append("svg")
    ...
    const xScale = d3.scaleLog().domain([xMin, xMax]).range([width, 0]);
    ...
    function updateGraph() {
      ...
      function semiLogLinearRegression(xData, yData) {
        ...
        const slope =
          (n * sumLogXY - sumLogX * sumY) /
          (n * sumLogXLogX - sumLogX * sumLogX);
        ...
        for (let i = 0; i < n; i++) {
          ...
          residualSS += Math.pow(
            yData[i] - (slope * logXData[i] + intercept),
            2
          );
        }
      ...
      console.log(
        "Calculated slope using (y2 - y1) / (log10(x2) - log10(x1)):",
        calculatedSlope
      );
      console.log(
        "Difference from regression slope:",
        Math.abs(calculatedSlope - result.slope)
      );
      ...
      const permeability =
        (162.2 *
          Calculation_data_array[0].Flow_rate *
          Calculation_data_array[0].Formation_Value_Factor *
          Calculation_data_array[0].Viscosity) /
        (Calculation_data_array[0].Height * absoluteSlope);

      decimaledPermeability = permeability.toFixed(1);
      console.log("permeability: ", decimaledPermeability);

      // Calculate skin factor
      const wellRadiusSquare =
        Calculation_data_array[0].Well_Radius *
        Calculation_data_array[0].Well_Radius;

      const yValue = result.slope * 1 + result.intercept;

      const LogPartSkinFactor = Math.log10(
        permeability /
          (Calculation_data_array[0].Porosity *
            Calculation_data_array[0].Viscosity *
            Calculation_data_array[0].Rock_Compressibility *
            wellRadiusSquare)
      );
      // 7.85
      const PressureSlopeSkinFactor =
        (y3 - Calculation_data_array[0].Initial_Pressure) / absoluteSlope;

      // calculate skinFactor
      const skinFactor =
        1.151 * (PressureSlopeSkinFactor - LogPartSkinFactor + 3.23);
      ...
      document.getElementById("calculations-container").innerHTML = `
            ...
            \\[
            = \\left| \\frac{${y1.toFixed(2)} - ${y2.toFixed(
        2
      )}}{\\log(10) - \\log(1)} \\right|
            \\]
            ...
            <p style="overflow-x: auto">
            \\[
            k = \\frac{162.6 q B \\mu }{m h} = \\frac{(162.6)(${
              Calculation_data_array[0].Flow_rate
            })(${Calculation_data_array[0].Formation_Value_Factor})(${
        Calculation_data_array[0].Viscosity
      })}{(${absoluteSlope})(${
        Calculation_data_array[0].Height
      })} = ${decimaledPermeability} \\text{ md.}
            \\]
            </p>
            ...
            <p style="overflow-x: auto">
            \\[
            s = 1.1513 \\left[ \\frac{(${y3.toFixed(2)}) - ${
        Calculation_data_array[0].Initial_Pressure
      }}{${absoluteSlope}} - \\log \\left( \\frac{${decimaledPermeability}}{(${
        Calculation_data_array[0].Porosity
      }) (${Calculation_data_array[0].Viscosity}) (${
        Calculation_data_array[0].Rock_Compressibility
      }) (${Calculation_data_array[0].Well_Radius})^2} \\right) + 3.23 \\right]
            \\]
            \\[
...
function drawdownPlot(time, Pressure_, arrayOfObjectsSheet2) {
  ...
  function Semi_LogGraph(time, Pressure_) {
    ...
    function updateGraph() {
      ...
      // 7.85
      const PressureSlopeSkinFactor =
        (Calculation_data_array[0].Initial_Pressure - y1) / absoluteSlope;
    ...
    \\[
    = \\left| \\frac{${y2.toFixed(2)} - ${y1.toFixed(
        2
      )}}{\\log(10) - \\log(1)} \\right|
    \\]
    ...
    s = 1.1513 \\left[ \\frac{(${
      Calculation_data_array[0].Initial_Pressure
    }) - ${y1}}{${absoluteSlope}} - \\log \\left( \\frac{${decimaledPermeability}}{(${
        Calculation_data_array[0].Porosity
      }) (${Calculation_data_array[0].Viscosity}) (${
        Calculation_data_array[0].Rock_Compressibility
      }) (${Calculation_data_array[0].Well_Radius})^2} \\right) + 3.23 \\right]
...
function Variable_RatePlot(time, Pressure_, arrayOfObjectsSheet2, FlowRate_) {
  ...
  const variable = variableRatePressure(
    pressure_array,
    time_array,
    flowRate,
    Calculation_data_array[0].Initial_Pressure
  ...
  function Semi_LogGraph(time, variable_data) {
    ...
    function renderYScaleAndData(yMin, yMax, original_Data) {
      ...
      svg
        ...
        .style("fill", "white")
        .html(
          '<div xmlns="http://www.w3.org/1999/xhtml" style="text-align: center; color: white;">' +
            "\\( \\frac{(p_i - p_{wf})}{q} \\)" +
            "</div>"
        );
...
\\]
            ...
            \\[
            = \\frac{${y2.toFixed(2)} \\text{psi/STB/D} - ${y1.toFixed(
        2
      )} \\text{psi/STB/D}}{\\log(10) - \\log(1)}
            \\]
            ...
            k = \\frac{162.6 q B \\mu }{m h} = \\frac{(162.6)(${
              Calculation_data_array[0].Formation_Value_Factor
            })(${Calculation_data_array[0].Viscosity})}{(${absoluteSlope})(${
        Calculation_data_array[0].Height
      })} = ${decimaledPermeability} \\text{ md.}
            \\]
            </p>
